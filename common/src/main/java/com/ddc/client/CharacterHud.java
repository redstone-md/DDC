 ...
 *
 * <p>Draws only what the server sent. It holds no rules and derives no numbers, so it cannot
 * disagree with the server about a character.
 *
...
public final class CharacterHud {
    ...
    private OptionalInt maxHitPoints = OptionalInt.empty();

    /** Takes the sheet the server sent. */
    public void accept(CharacterSheet sheet, OptionalInt maxHitPoints) {
        this.sheet = sheet;
    ...
    public void render(GuiGraphicsExtractor graphics, Font font) {
        ...
        graphics.text(font, header, MARGIN + PADDING, MARGIN + PADDING, hitPointColour());
        graphics.text(font, abilities, MARGIN + PADDING, MARGIN + PADDING + LINE_HEIGHT, TEXT);
    }

    private String headerText() {
        StringBuilder sb = new StringBuilder("LVL ").append(sheet.level())
                .append("  HP ").append(sheet.currentHitPoints());
        maxHitPoints.ifPresent(max -> sb.append('/').append(max));
        return sb.append("  PROF +").append(sheet.proficiencyBonus()).toString();
    }
    ...
    private int hitPointColour() {
        if (maxHitPoints.isEmpty()) {
            return TEXT_DIM;
        }
        int max = maxHitPoints.getAsInt();
        if (max <= 0) {
            return TEXT_DIM;
        }
        double fraction = (double) sheet.currentHitPoints() / max;
        if (fraction <= 0.25) {
            return HP_CRITICAL;
        }
